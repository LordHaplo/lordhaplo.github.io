...
import { TRY_BUY, USE, TRY_USE, TRY_USE_ON, EVT_STAT } from '../events';
...
export default {
	...
	components:{
		...
		familiars:()=>import( /* webpackChunkName: "familiars-ui" */ './sections/familiars.vue' ),
	...
	methods:{
		...
		initEvents() {
			...
			this.add( TRY_USE_ON, this.onUseOn );
		...
		startAutoSave() {
			...
			if ( Settings.getSubVar( 'saves', 'autoSave') && !this.saver ) {
...
<!-- end popups -->
		...
		<template slot="sect_familiars">
			<familiars :module="state.getModule('familiars')" />
		</template>
